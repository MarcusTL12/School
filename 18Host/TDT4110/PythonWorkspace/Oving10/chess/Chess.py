...
def run():
	amt_players = int(input("How many real players"))
	game = Board()
	turn = True
	...
	while True:
		...
		if (amt_players >= 1 and turn) or (amt_players >= 2 and not turn):
			mov = prompt_move(game)
		else:
			i = random.randint(0, len(game.all_moves[0]) - 1)
			mov = game.all_moves[0][i], game.all_moves[1][i]
			input()
